...
public class TaskOrganizerApplication {
	...
	public CommandLineRunner taskDemo(TaskRepository trepository, CategoryRepository crepository, UserRepository urepository) {
		return (args) -> {
			...
			trepository.save(new Task("Final project", "Finish final project", "5.5.22", crepository.findByName("Backend").get(0), user2));
			trepository.save(new Task("Complete documentation", "Write necessary documentation", "4.5.22", crepository.findByName("Softala").get(0), user2));	
